...
- [ ] **Step 1.1:** Create a `codex/public-positioning-ponytail` worktree from
  the approved plan commit using the worktree skill.
- [ ] **Step 1.2:** Confirm the worktree starts clean with `git status -sb`.
- [ ] **Step 1.3:** Confirm `~/.config/ponytail/config.json` still contains
  `"defaultMode": "off"`; do not edit it.
- [ ] **Step 1.4:** Run `pnpm install --frozen-lockfile`.
- [ ] **Step 1.5:** Run the baseline commands `pnpm typecheck`, `pnpm test`, and
...
- [ ] **Step 2.1:** Write `integrations/ponytail.md` with these exact policy
...
- [ ] **Step 2.2:** Include a beginner analogy and a mode table that does not
  describe `lite` as "almost off."
- [ ] **Step 2.3:** Include the isolated comparison sequence and the measurements
  from SPEC-0004.
- [ ] **Step 2.4:** Update `integrations/superpowers.md` so responsibility order
  is PGS routing, Superpowers workflow, then optional Ponytail minimization.
- [ ] **Step 2.5:** Search both files for accidental claims that either plugin is
...
- [ ] **Step 3.1:** Create the reference with valid doc-gov frontmatter and a
  beginner-first comparison table.
- [ ] **Step 3.2:** Separate required PGS packages, recommended engineering
  workflow tools, and optional complexity advisers.
- [ ] **Step 3.3:** Recommend Superpowers for engineering/runtime work while
  retaining the current rule that doc-only work does not receive full
  engineering ceremony by default.
- [ ] **Step 3.4:** Recommend Ponytail installation with global `off` and
  explicit session/task activation only.
- [ ] **Step 3.5:** Link the reference from the adoption playbook and public site
  brief without duplicating the full rule body.
- [ ] **Step 3.6:** Run `pnpm doc-gov scan` to refresh the manifest.
...
- [ ] **Step 4.1:** Preserve the CI and npm badges and add a six-language switcher
  directly below the title/badges.
- [ ] **Step 4.2:** Replace the architecture-first opening with one answer-first
  paragraph: PGS keeps long-running AI-assisted projects understandable and
  governable.
- [ ] **Step 4.3:** Explain the pain before the components: AI creates useful
  artifacts faster than teams can organize, retire, and trust them.
- [ ] **Step 4.4:** Present the librarian, traffic desk, and inspection-machine
  model before introducing doc-gov/pro-gov names.
- [ ] **Step 4.5:** Move a short evaluation path before detailed repository and
  lifecycle references.
- [ ] **Step 4.6:** Confirm the first screen answers what PGS is, who it helps,
  and why it matters.
...
- [ ] **Step 5.1:** Explain CLI as "a command you or an AI can run" on first use.
- [ ] **Step 5.2:** Explain governance as "rules that keep current truth easy to
  find and old work from pretending to be current."
- [ ] **Step 5.3:** Add one concrete before/after project story showing plan and
  document clutter becoming a governed current-work path.
- [ ] **Step 5.4:** Add one Mermaid relationship diagram with plain-language
  labels for Git, PGS, Superpowers, and Ponytail.
- [ ] **Step 5.5:** Break long text walls into tables, examples, and short
  paragraphs without deleting technical reference material.
- [ ] **Step 5.6:** Search for unexplained `CLI`, `SSOT`, `hook`, `manifest`,
...
- [ ] **Step 6.1:** Cross-check every named command against CLI source/help.
- [ ] **Step 6.2:** Verify the package, ProjectLens, agent-assets, profile, and
  write-mode boundaries against code and policy.
- [ ] **Step 6.3:** Strengthen truthful highlights and remove claims of automatic
  migration, guaranteed savings, or plugin replacement.
- [ ] **Step 6.4:** Verify every relative link from the root README resolves.
- [ ] **Step 6.5:** Record the three README passes in the implementation commit or
  final report.
...
- [ ] **Step 7.1:** Give the npm README the same short answer, pain, and beginner
  analogy as the root README.
- [ ] **Step 7.2:** Keep package install, public commands, checkout-only commands,
  read-only behavior, and package boundary details.
- [ ] **Step 7.3:** Link to the canonical GitHub README for the full introduction.
- [ ] **Step 7.4:** Update the publication brief so an external website does not
...
- [ ] **Step 8.1:** Freeze the reviewed English heading order and link targets.
- [ ] **Step 8.2:** Write Simplified Chinese with natural beginner language, not
  word-for-word machine phrasing.
- [ ] **Step 8.3:** Write Japanese with natural technical vocabulary and the
  same beginner explanations.
- [ ] **Step 8.4:** Write Spanish, French, and German with equivalent claims,
  examples, commands, and boundaries.
- [ ] **Step 8.5:** Put the same mutual language switcher at the top of all six
  files.
- [ ] **Step 8.6:** State in every translation that English is canonical when
  wording drifts.
- [ ] **Step 8.7:** Keep code blocks, package names, file paths, and commands
...
- [ ] **Step 9.1:** Run a Node one-off check that all six files exist and contain
  links to all six language files.
- [ ] **Step 9.2:** Run a Node one-off check that all six files contain the same
  number of level-2 headings, fenced code blocks, and Mermaid blocks.
- [ ] **Step 9.3:** Compare package-name and command inventories across all six
  files; expected result is no missing command in any translation.
- [ ] **Step 9.4:** Run `git diff --check` to catch Markdown whitespace damage.
- [ ] **Step 9.5:** Manually inspect the beginning, example, quick start, boundary,
...
- [ ] **Step 10.1:** Invoke `doc-cross-validator` and read its full instructions.
- [ ] **Step 10.2:** Verify README and integration claims against package JSON,
  CLI source, Ponytail 4.7.0 source/config, Superpowers integration, and current
  npm/GitHub metadata.
- [ ] **Step 10.3:** Correct every factual mismatch in the canonical English
  source first, then propagate the correction to all translations.
- [ ] **Step 10.4:** Run `pnpm doc-gov scan` after governed-doc edits.
- [ ] **Step 10.5:** Run `pnpm doc-gov router-check`, `check`, `scan --check`,
...
- [ ] **Step 11.1:** Run `pnpm install --frozen-lockfile`.
- [ ] **Step 11.2:** Run `pnpm typecheck`; expected result is both packages pass.
- [ ] **Step 11.3:** Run `pnpm test`; expected result is all doc-gov and pro-gov
  tests pass.
- [ ] **Step 11.4:** Run `pnpm build`; expected result is both CLIs build and
  new integration/reference files appear under generated pro-gov assets.
- [ ] **Step 11.5:** Run `node packages/pro-gov/dist/cli.js doctor`.
- [ ] **Step 11.6:** Run both package `pack --dry-run` commands.
- [ ] **Step 11.7:** Inspect the pro-gov tarball manifest: version `0.3.4`,
  dependency `@pieai/doc-gov ^0.3.3`, Ponytail integration included, private
  and third-party agent bodies excluded.
- [ ] **Step 11.8:** Install the packed tarball into a temporary project and run
  `pro-gov --help`, `pro-gov assets list`, and `pro-gov doctor`.
...
- [ ] **Step 12.1:** Mark completed plan checkboxes as evidence accumulates.
...
- [ ] **Step 13.4:** Wait for the exact-head `docs-check` GitHub Actions run.
- [ ] **Step 13.5:** Inspect failed logs if any step fails; do not publish until
...
- [ ] **Step 14.1:** Confirm public npm still reports pro-gov `0.3.3` and no
  existing `0.3.4`.
- [ ] **Step 14.2:** Authenticate through the user's already logged-in Edge npm
  session without reading or transmitting credentials.
- [ ] **Step 14.3:** Publish only `@pieai/pro-gov@0.3.4`; do not republish
  unchanged `@pieai/doc-gov@0.3.3`.
- [ ] **Step 14.4:** Poll the official registry until `npm view
  @pieai/pro-gov version` returns `0.3.4`.
- [ ] **Step 14.5:** Install `@pieai/pro-gov@0.3.4` from the public registry in a
