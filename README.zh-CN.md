...
[Compound Engineering](integrations/compound-engineering.md) 和可选的
...
| Compound Engineering | 菜谱笔记本 | 通过 `ce-compound` 在验证后的工作里沉淀可复用经验。 |
...
flowchart LR
  ...
  D --> E["Compound Gate 判断是否沉淀经验"]
  E --> F["PGS 保存长期证据"]
  F --> G["Git 记录历史"]
```

PGS 决定**工作应该放在哪里、需要走哪条路线**；Superpowers 决定**工程工作怎样
有纪律地进行**；Ponytail 可以追问**实现能否更精简**；Compound Engineering 可以在
工作验证后记录**下次还能复用的经验**；Git 记住最终发生的修改。
...
PGS 可以通过用户自己的 portfolio manifest 检查多个仓库，但公开包不会携带任何真实用户
的项目列表或私有控制仓库。
...
`pro-gov portfolio check|plan|assets-check|doctor` 只读取这个显式配置。如果没有提供完整本地 PGS checkout，
它会使用 `@pieai/pro-gov` 包里已经审查过的 public assets。
`portfolio doctor` 是离线只读的车队总检查，不会更新插件或修改目标仓库。

## 这些部分怎样配合

1. AI 先读取 `AGENTS.md`。
2. 非平凡工作开始前，PGS Learning Recall 会从 `docs/solutions/**` 和
   `CONCEPTS.md` 召回相关历史经验。
3. PGS 从 `docs/governance/agents-routing/` 选择合适的 profile 和工作路线。
4. 需要工程纪律时，Superpowers 在这条路线内执行。
5. 需要一次受控的简化审查时，才明确调用 Ponytail。
6. 需要长期保留的需求说明、计划、决策和参考资料进入受治理的位置。
7. `doc-gov` 和 `pro-gov doctor` 检查这套系统是否真的接好，而不只是文档里说接好了。
...
预览模式是只读的。全新项目安装包后可以使用 `init --apply`；如果任何目标文件已经
存在，它会在写入前整体停止。已有项目继续使用 dry-run 结果做人工迁移，PGS 不会偷偷
重写项目自己的事实。
...
Compound Engineering 推荐用于工作完成后的经验沉淀。在 PGS 治理的项目里，默认不是让
CE 和 Superpowers 抢主流程；默认是在验证后的工作末尾走一次 Compound Gate，只有确实
有可复用经验时才运行 `ce-compound`。

PGS Learning Recall 是这条闭环的开工前半段。非平凡实现、调试、发布、架构或
portfolio 同步工作开始前，运行：

```bash
pro-gov learn recall --query "<任务摘要>"
```

先读相关命中，再改文件。它复用 CE 写下来的 `docs/solutions/**` 经验，但不会让
CE 变成默认执行流程。
...
- 取代 Git、`AGENTS.md`、Superpowers、Compound Engineering 或 Ponytail；
- 自动理解并重写每一个项目；
...
| `integrations/` | 与 Superpowers、Compound Engineering、Ponytail 和 Directed Development 的边界。 |
...
| 它会取代 Compound Engineering 吗？ | 不会。CE 可以提供工作后的 `ce-compound` 经验沉淀尾巴；完整 CE 流程仍然需要明确指定。 |
| 应该全局打开 Ponytail 吗？ | 不应该。保持 `off`，先在隔离任务里测试 `lite`。 |
| `pro-gov init` 会覆盖项目吗？ | 不会。`--apply` 只用于全新目标；任何目标文件已存在时都会在写入前整体停止。 |
