...
Loop: `observe -> change the smallest useful thing -> verify -> if wrong,
re-diagnose before stacking patches`. After three failed patches, stop and switch
...
`playwright-extension` is the fallback for the real-profile lane when a host has
no first-party extension, or its extension is not connected.

Copilot CLI is offboarded for Yuanfei's normal workflow. Do not route browser,
real-profile, or agent delegation work to Copilot unless Yuanfei explicitly asks
for Copilot in the current thread and confirms the account is usable.
