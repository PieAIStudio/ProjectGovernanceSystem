...
name: web-search
description: >-
  Free web search using dokobot read in local mode — read search engine result pages through your real Chrome browser. No API key, no cost, no rate limits. Works with Google, Bing, DuckDuckGo, Baidu, and any search engine.
read_when:
...
Use whichever engine suits the query. Google is the default choice for most searches. For Chinese content, Baidu or Sogou may return better results.
...
- Some search engines load results dynamically — dokobot handles JavaScript rendering, but infinite-scroll results may require scrolling
