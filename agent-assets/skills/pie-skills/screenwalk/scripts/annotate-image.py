#!/usr/bin/env python3
"""Draw simple numbered boxes on an image for owner review briefs."""
...
def draw_annotations(image_path: Path, output_path: Path, annotations: list[dict[str, Any]]) -> None:
    ...
    for index, item in enumerate(annotations):
        ...
        badge_h = 22
        draw.rectangle([x, max(0, y - badge_h), x + badge_w, y], fill=color)
        draw.text((x + 6, max(0, y - badge_h) + 5), label, fill=(0, 0, 0, 255), font=font)
