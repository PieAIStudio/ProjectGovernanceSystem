...
**所有层级**：
- 禁止用"简单来说"开头
- 禁止一整段文字没有任何视觉元素（图表、表格、引用块）
- 禁止写完不做交叉验证
- 禁止在没有比喻或例子的情况下引入复杂概念

**初级额外禁止**：
- 禁止假设读者知道什么是 CLI、JSON、hash、config、commit
- 禁止在正文中直接列命令行
...
**高级额外禁止**：
- 禁止只说"怎么做"不说"为什么这样设计"
- 禁止把边界条件和失败模式当作"不重要"省略

## 质量标准

| 维度 | 🟢 初级 | 🟡 中级 | 🔴 高级 |
| --- | --- | --- | --- |
| 比喻密度 | 每个核心概念至少 1 个 | 复杂概念必须有 | 设计决策必须有 |
| 图表密度 | 每篇至少 3 个 | 每篇至少 2 个 | 每篇至少 2 个 |
| 操作示例 | 自然语言对话表格 | 对话 + 命令对照 | 命令 + 输出 + 解读 |
| FAQ | 每篇结尾必须有 | 每篇结尾必须有 | 每篇结尾有 + 自测题 |
| 术语处理 | 全部用人话替代 | 系统术语 + 人话注释 | 精确定义 + 功能边界 |
| 连贯性 | 虚拟人物贯穿 | 虚拟人物贯穿 | 场景贯穿 |
| 阅读时间 | 每篇 8-12 分钟 | 每篇 10-15 分钟 | 每篇 10-20 分钟 |
