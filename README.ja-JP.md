...
[Compound Engineering](integrations/compound-engineering.md)、任意の
...
| Compound Engineering | レシピノート | 検証済み作業のあと `ce-compound` で再利用できる学びを残す。 |
...
flowchart LR
  ...
  D --> E["Compound Gate が学びを残すか判断"]
  E --> F["PGS が長期的な証拠を保存"]
  F --> G["Git が履歴を記録"]
```

PGS は**作業の置き場所と経路**を決めます。Superpowers は**規律ある実装手順**を
担当します。Ponytail は**もっと簡潔にできるか**を問い、Compound Engineering は
検証後に**再利用できる学び**を残せます。Git は実際の変更を記録します。
...
`pro-gov portfolio check|plan|assets-check|doctor` はこの明示的な設定だけを読みます。完全なローカル PGS
checkout が指定されていなければ、`@pieai/pro-gov` に同梱されたレビュー済み public
assets を使います。
`portfolio doctor` はオフラインの読み取り専用フリート検査で、プラグイン更新や対象リポジトリの変更は行いません。
...
Compound Engineering は作業後の知識捕捉に推奨されます。PGS 管理プロジェクトでは、
既定で CE を Superpowers と競わせません。検証済み作業のあと Compound Gate として使い、
再利用できる学びがある場合だけ `ce-compound` を実行します。
...
- Git、`AGENTS.md`、Superpowers、Compound Engineering、Ponytail の置き換え。
...
| `integrations/` | Superpowers、Compound Engineering、Ponytail、Directed Development との境界。 |
...
| Compound Engineering を置き換えますか？ | いいえ。CE は作業後の `ce-compound` 学習の尾を提供できます。完全な CE フローは明示指定のままです。 |
